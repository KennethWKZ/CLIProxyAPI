...
func TestCodexCredsTrimsTrailingSlashFromBaseURLOverrides(t *testing.T) {
	_, authBaseURL := codexCredsWithConfig(&config.Config{CodexBaseURL: "http://127.0.0.1:18082/"}, &cliproxyauth.Auth{
		Attributes: map[string]string{"base_url": "http://127.0.0.1:18102/"},
		Metadata:   map[string]any{"access_token": "oauth-token"},
	})
	if authBaseURL != "http://127.0.0.1:18102" {
		t.Fatalf("auth baseURL = %q, want trimmed trailing slash", authBaseURL)
	}

	_, metadataBaseURL := codexCredsWithConfig(&config.Config{CodexBaseURL: "http://127.0.0.1:18082/"}, &cliproxyauth.Auth{
		Metadata: map[string]any{"access_token": "oauth-token", "base-url": "http://127.0.0.1:18104/"},
	})
	if metadataBaseURL != "http://127.0.0.1:18104" {
		t.Fatalf("metadata baseURL = %q, want trimmed trailing slash", metadataBaseURL)
	}

	_, globalBaseURL := codexCredsWithConfig(&config.Config{CodexBaseURL: "http://127.0.0.1:18082/"}, &cliproxyauth.Auth{
		Metadata: map[string]any{"access_token": "oauth-token"},
	})
	if globalBaseURL != "http://127.0.0.1:18082" {
		t.Fatalf("global baseURL = %q, want trimmed trailing slash", globalBaseURL)
	}
}

func TestCodexCredsSkipsGlobalBaseURLForAPIKeyAuthKindAttribute(t *testing.T) {
	_, baseURL := codexCredsWithConfig(&config.Config{CodexBaseURL: "http://127.0.0.1:18082"}, &cliproxyauth.Auth{
		Attributes: map[string]string{"auth_kind": "api-key"},
		Metadata:   map[string]any{"access_token": "sk-codex-test"},
	})
	if baseURL != "" {
...
func TestResolveCodexConfigNormalizesTrailingSlashForBaseURLMatch(t *testing.T) {
	exec := NewCodexExecutor(&config.Config{
		CodexKey: []config.CodexKey{{APIKey: "key-123", BaseURL: "http://127.0.0.1:18102/"}},
	})
	...
	entry := exec.resolveCodexConfig(auth)
	if entry == nil {
		t.Fatal("expected trailing-slash base URL to match Codex key config")
	}
	if entry.APIKey != "key-123" {
		t.Fatalf("entry.APIKey = %q, want key-123", entry.APIKey)
	}
}
