...
// AuthKindFromAttributes classifies an auth from its attributes alone and
// returns an empty string when the attributes carry no verdict. Unlike
// Auth.AuthKind it never reads Auth.Metadata, so request-path callers can
// classify a credential while a token refresh mutates that map.
func AuthKindFromAttributes(auth *Auth) string {
	if kind := normalizeAuthKind(authAttribute(auth, AttributeAuthKind)); kind != "" {
		return kind
	}
	if authAttribute(auth, AttributeAPIKey) != "" {
